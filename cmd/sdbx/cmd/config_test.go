...
func TestConfigSet(t *testing.T) {
	...
	if !strings.Contains(output, "Runtime files regenerated") {
		t.Errorf("Output should mention regeneration: %s", output)
	}

	sdbxConfig := readCommandTestFile(t, filepath.Join(projectDir, ".sdbx.yaml"))
	if !strings.Contains(sdbxConfig, "domain: newdomain.example.com") {
		t.Fatalf(".sdbx.yaml did not contain new domain:\n%s", sdbxConfig)
	}
	routes := readCommandTestFile(
		t,
		filepath.Join(projectDir, "configs", "traefik", "dynamic", "middlewares.yml"),
	)
	if !strings.Contains(routes, ".newdomain.example.com") {
